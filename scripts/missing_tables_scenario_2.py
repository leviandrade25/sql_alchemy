...
def generate_table_drop_script(tabla):
    print("generate_table_drop_script")
    script = f"DROP TABLE {tabla.schema}.{tabla.name}"
    print(script)
    return script

def drop_aditional_tables_in_database_standard(metadata_standard, metadata_cloned, conn_cloned, database_standard):
    print("drop_aditional_tables_in_database_standard")
    ...
    tables_to_drop = set(tables_database_cloned) - set(tables_database_standard)
    ...
    for tabla in tables_to_drop:
        # Obtener definición de tabla desde metadata_standard
        table_definition = get_table_definition(metadata_cloned, tabla, conn_cloned)
        
        
        # Generar script de creación de tabla
        table_drop_script = generate_table_drop_script(table_definition)

         
        drop_scripts.append(table_drop_script)
        # Ejecutar el script en metadata_cloned
        conn_cloned.execute(DDL(table_drop_script))
        conn_cloned.commit()
        


        print(f"Table {tabla} dropped in {tables_database_standard}")
...
def main():
    ...
    for schema in set(schemas_standard) & set(schemas_cloned):
        ...
        creation_scripts = drop_aditional_tables_in_database_standard(metadata_standard, metadata_cloned, conn_clonned, database_cloned)
